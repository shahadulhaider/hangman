...
var guessedLetters = [];
var currentFruit = '';
var fruitsList = ["apple", "apricot", "avocado", "banana", "blackberry", "blackcurrant", "blueberry", "cantaloupe", "cherry", "clementine", "coconut", "cranberry", "cucumber", "currant", "date", "dragonfruit", "durian", "elderberry", "eggplant", "fig", "gooseberry", "grape", "grapefruit", "guava", "huckleberry", "jackfruit", "kiwifruit", "lemon", "lime", "loquat", "longan", "lychee", "mandarine", "mango", "mulberry", "nectarine", "olive", "orange", "papaya", "passionfruit", "peach", "pear", "persimmon", "plantain", "plum", "pineapple", "pomegranate", "pomelo", "pumpkin", "raspberry", "squash", "strawberry", "tamarind", "tangerine", "tomato", "watermelon"]; // Divs
...
var button = document.querySelector('button'); // Init function

function initialize() {
  currentFruit = fruitsList[Math.round(Math.random() * fruitsList.length)].toUpperCase();
  guessesRemaining = currentFruit.length + 5;
  guessDiv.innerHTML = "Guess Left: ".concat(guessesRemaining);
  guessedLetters = [];
  guessedDiv.textContent = 'Ready, Set, Go!';
  scoreDiv.innerHTML = "Wins: ".concat(winCount, " | Losses: ").concat(lossCount); // hangman
  ...
  cheaterDiv.textContent = 'You cheater! The fruit is: ' + currentFruit; // Finally start the game
...
document.addEventListener('keydown', function (event) {
  if (init === 1 && event.which >= 65 && event.which <= 90) {
    ...
    if (emptyWord.join('') === currentFruit) {
      winCount++;
      scoreDiv.innerHTML = "Wins: ".concat(winCount, " | Losses: ").concat(lossCount);
